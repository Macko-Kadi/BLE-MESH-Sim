...
//	public static final String algorithm = "Prim";
	...
	public static final float BATTERY_CHECK_INTERVAL = 300f; // how often to update battery statuses (and therefore ->
	...
	static final float MAX_SIM_TIME = 360f; // 6 minuts in the simulated world
	...
	// ==========================================JUST AN EXEMPLARY ENVIRONMENT====================================================
			...
			for (Map.Entry<Float, Float> entry2 : tmp.entrySet()) 	{
				...
				float[] sourcePar = { 1f / 60f }; // each node has Poisson source (type 0) with lambda=1/60 (once per
//				float[] sourcePar = { 1f / 6f };		// minute)
				 ...
				 * triggered in the example:
				 */
				
				if(entry1.getKey()!=1)
					eventList.addEventsFromList(LIST_OF_NODES.get(entry1.getKey()).generatePacket());
			}
			}
		...
		final Medium medium = new Medium(); // we use only static methods, so ignore the warning. However, we need to
											...
											// on.
		// same as with packet generation - first BATTERY_CHECK event must be triggered
		// manually
...
//		eventList.addEvent(new Event(0.017088f, "GO_SLEEP", Byte.toString((byte) 5)));
	...
	// ==========================================EDN OF: JUST AN EXEMPLARY ENVIRONMENT============================================
		 ...
		 */
		while (simTime < MAX_SIM_TIME && !(eventList.theList.size() == 1)) { // simTime must not exceed range, and there
																				// must be an event to evaluate. Why
																				// ==1, not 0? Because there is always
